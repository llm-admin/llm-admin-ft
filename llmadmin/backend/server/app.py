...
import llmadmin.backend.server.config as CONFIG
...
class LLMDeployment(LLMPredictor):
    ...
    def update_batch_params(self, new_max_batch_size:int, new_batch_wait_timeout_s:float):
        ...
        logger.info(new_max_batch_size)
        logger.info(new_batch_wait_timeout_s)
...
class ExperimentalDeployment(GradioIngress):
    ...
    async def query(self, *args) -> Dict[str, Dict[str, Any]]:
        ...
        logger.info(prompts)
        results = await asyncio.gather(
            *(await asyncio.gather(*[self._model.generate.remote(Prompt(prompt=prompts, use_prompt_format=False))]))
        )
    ...
    def _chose_ui(self) -> Callable:
        print(self._model_configuration.model_config.model_task)
